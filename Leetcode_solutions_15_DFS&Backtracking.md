...
千万不要把 $O(M \times N \times 3^L)$ 或者 $O(V + E)$ 随便套给所有的 DFS 题目。事实上，DFS 题目的时间复杂度，完全取决于它的**“决策树是怎么画的”**。
...
* **时间复杂度**：$$O(N \times N!)$$
* **算理分析**：决策树的叶子节点个数是 $N!$（阶乘）。因为每次走到叶子节点，都需要花 $O(N)$ 的时间把当前排列复制到结果集里，所以是 $O(N \times N!)$。
