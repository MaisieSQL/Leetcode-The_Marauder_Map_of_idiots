...
## ⚠️ 重点避坑：为什么不能直接返回 `dp[n-1]`？

> **常见疑惑**：既然是 DP，为什么不直接返回最后一个格子 `dp[n-1]`，而必须在遍历时记录 `max_sum = max(max_sum, dp[i])`？

* **原因在于 `dp[i]` 的严格定义**：
  * `dp[n-1]` 仅仅代表**必须包含数组最后一个数字 `nums[n-1]`** 的子数组最大和。
  * 但全局最大和的那个子数组，**不一定会刚好停在数组的最后一个元素**！它可能在数组中间就达到了峰值。
* **反例验证**：以 `nums = [1, 2, 3, -10, 1]` 为例：
  * `dp[2] = 6`（子数组 `[1, 2, 3]`，真正的全局最大值 🌟）
  * `dp[3] = -4`
  * `dp[4] = 1`（最后一个格子 `dp[n-1]`）
  * 如果直接返回 `dp[4]` 得到的是 `1`（错误！）；只有全程维护 `max_sum` 才能成功锁住峰值 `6`。
...
class Solution:
    def maxSubArray(self, nums: list[int]) -> int:
        ...
        for i in range(1, n):
            dp[i] = max(nums[i], dp[i - 1] + nums[i])
            max_sum = max(max_sum, dp[i])  # 随时记录历史最高峰
