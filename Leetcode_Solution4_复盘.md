...
> **流向暗号：** `A ── ➔ B` 代表 A 题练就的底层内功，可以直接向右降维打击 B 题！
...
21.合并两个有序链表 `[已通关 🏆]` (双线推进拉拉链合并，尾部一刀直接无循环缝合)
   └── ➔ 19.删除倒数第N个节点 `[已通关 🏆]` (双指针恒定间距为 N，前哨踩空后卫刚好卡在目标前驱)
        └── ➔ 86.分隔链表 `[未解锁 🔒]` (开辟小、大两条 dummy 分头接人，最后火下一秒连线)
   ...
   └── ➔ 543.二叉树的直径 `[未解锁 🔒]` (利用求深度的后序位，顺手在当前层刷新左右高度之和的最大值)
        └── ➔ 226.翻转二叉树 `[已通关 🏆]` (镜像对称，左右子树无脑原地对调，大老板汇报)
             └── ➔ 100.相同的树 `[已通关 🏆]` (多路同步安检，双空、单空、值不等三道生死线横向卡死)
                  └── ➔ 101.对称二叉树 `[已通关 🏆]` (镜像探戈，利用 check 辅助函数拓展双参数实现外侧对比和内侧对比)
                       └── ➔ 112.路径总和 `[未解锁 🔒]` (自上而下顺藤摸瓜做减法，触底看叶子是否扣完)
                            └── ➔ 236.最近公共祖先 `[已通关 🏆]` (面霸级，极致后序流，左右两路皆传捷报，大老板当中当场拦截)
...
**1. BST 极速二分导航与验证主线：**
700.二叉搜索树中的搜索 `[已通关 🏆]` (左小右大黄金导航，明确 While 循环原地新值盖旧值才是真迭代)
   └── ➔ 98.验证二叉搜索树 `[已通关 🏆]` (看穿局部父子对比的漏洞，前序遍历大权独揽，将全局边界上下限区间传给底层)

**2. BST 中序遍历特性与器官移植主线：**
230.BST中第K小的元素 `[已通关 🏆]` (手操物理栈中序迭代，指针 `curr` 控向，`stack.pop()` 在物理上就是傲然登场的【根】)
...
102.二叉树的层序遍历 `[已通关 🏆]` (外层大轮盘用 deque，提前拍照锁死 `queue_size` 发放限定数量的入场券)
   └── ➔ 107.层序遍历 II `[未解锁 🔒]` (自底向上的层序，最终大账本反转或者直接头部塞入)
        └── ➔ 103.锯齿形层序遍历 `[已通关 🏆]` (利用双端队列 `appendleft()` 的 O(1) 头部插入开挂超能力，优雅实现偶数层原地反转)
