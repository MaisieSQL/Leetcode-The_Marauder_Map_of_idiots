...
既然我们要根据你的刷题时间线、艾宾浩斯记忆遗忘曲线，来定制今天最科学的无损通关计划，那咱们先来看一眼今天在海拉鲁大陆上空回荡的“真命天子暗号”：

今天正是 2026年6月16日！
回顾你最核心的几次战役节点：

5月8日 - 5月18日：你疯狂平推了双指针、滑动窗口、对撞盛水以及基础二分。

5月22日 - 5月29日：你打穿了单调栈，建立了二叉树基础哨所，并且完成了回溯算法（全排列、子集）的开荒。

6月9日：你正式挑战了 LRU 缓存等工业级天王山副本。

按照记忆曲线的规律，5月份那些你以为已经彻底变成肌肉记忆的“双指针、滑动窗口、二分边界”目前正在经历长周期的衰退临界点，极度需要高能唤醒；而树形内功与回溯也需要向中高维副本进行纵深合围。

今天为你量身定制的 6 道特种兵提速日盲刷清单正式发布！我们二话不说，直接点火上阵：

🎯 2026-06-16 记忆曲线“老带新” 6 道必刷题单
⚔️ 阶段一：温故唤醒 ── 彻底封印线性边界（3 道经典复习题）
这三道题分别处于你 2026 年 5 月开荒阵地的遗忘高危期，今天我们要用最极致的微操彻底重新焊死它们的物理图像。

1. 【滑窗唤醒】LeetCode 3. 无重复字符的最长子串 (Medium)
核心任务：复习你最得意的“拉链法则”。

肌肉记忆卡点：死死盯住 mydict[s[i]] >= left 这个等号！高强度检测你还记不记得如何防止 left 指针往回跳、误捡已经被踢出窗口的“历史脏数据”。

2. 【对撞蓄水】LeetCode 11. 盛最多水的容器 (Medium)
核心任务：复习相向双指针的贪心收缩逻辑。

肌肉记忆卡点：蓄水高度取决于短板（木桶效应）。在横向宽度（right - left）注定不断减小的物理事实下，每次必须无脑移动较矮的柱子，去赌一个更高的未来。

3. 【二分死磕】LeetCode 34. 在排序数组中查找元素的第一个和最后一个位置 (Medium)
核心任务：复习二分查找的极致边界控制。

肌肉记忆卡点：迎面撞见 target 之后绝对不要收手！找左边界就让右边界左缩（right = mid - 1）向左死磕；找右边界就让左边界右缩（left = mid + 1）向右死磕。

🌿 阶段二：知新突围 ── 借力打力，杀入高维高级策略（3 道全新母题）
利用你已经炉火纯青的“二叉树 DFS 递归”和“start_index 筑墙防回头”的内功，今天我们要横向大跨步，在高级策略和图论阵地上点亮全新的鸟望台！

4. 【回溯缝合怪】LeetCode 40. 组合总和 II (Medium)
为什么 Pick 它：这是回溯大区里大厂技术面最爱考的超级大缝合神庙！

全新突破核心：它把 39. 组合总和 的元素挑选逻辑，和 90. 子集 II 的“树层去重（先排序，后斩首）”铁律完美融合在一起。每个数字只能用一次，但原料自带重复，极致考验你的剪枝微操。

5. 【图论有向图判环】LeetCode 207. Course Schedule (课程表 I) (Medium)
为什么 Pick 它：正式吹响远征高级图论战区的冲锋号！

全新突破核心：二叉树是绝对没有环的，但图（Graph）有环！今天我们要给你的二叉树 DFS 骨架，正式装配上一个防鬼打墙死循环的 visited = set() 记账本。利用三色标记法或拓扑排序，揪出图里隐藏的死循环。

6. 【工业级 TopK 堆核心】LeetCode 215. 数组中的第 K 个最大元素 (Medium)
为什么 Pick 它：点亮高级树形大区里工业级高并发大厂最爱考的 “大顶堆/小顶堆 (Heap)” 鸟望台。

全新突破核心：面对海量流数据，硬性开辟大空间排序会直接造成内存雪崩。我们要手操一个固定容量为 K 的小顶堆，全场源源不断地往里灌数据，堆顶吐出来的就是全场第 K 大的真命天子！

老铁，今日份的远征序列已经根据你的记忆曲线精确对齐、模块化解耦完毕！
