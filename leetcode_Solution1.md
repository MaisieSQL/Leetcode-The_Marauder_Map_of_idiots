...
# 巩固单链表 $\rightarrow$ 进阶双向链表 $\rightarrow$ 掌握循环链表

以下我为你精选的 3 道最具代表性的经典题目，它们能把你的“三指针策略”发挥到极致！第一步：复习与巩固单向链表
...
```
💡 核心大白话总结
单向链表：最后一个节点的 next 是 None（有终点）。

循环链表：最后一个节点的 next 是 head（首尾相接，无终点）。

所以，你不需要去背一个新的“循环链表定义”。只要你记住了单向链表和双向链表的定义，在需要循环的时候，用最后一根线把 tail.next 和 head 缝合在一起，它就自动变成循环链表了！

# 传说中的三指针解法。。。是解决链表问题的主要方法

---

# 🏆 LeetCode 算法战果盘点与技能树大地图

截至目前，你已经合力斩获了 **15 道超高频经典大厂面试题**（涵盖 Easy 到 Hard 级别的核心母题）。以下将这些战果按照 LeetCode 官方标准 **Tag（标签分类）** 进行了严格归类，方便一眼看清自己的技能树加点：

---

## 📊 你的算法技能树盘点 (按 Tag 标准分类)

### 🏷️ Tag 1: 哈希表 (Hash Table / Dictionary) ── 4 道题
* **核心内功**：利用哈希表 $O(1)$ 的极致查找速度，玩转“空间换时间”或“动态记账”。
* **战果清单**：
  1. **`1. Two Sum`**（乱序凑对，字典当备忘录）
  2. **`496. 下一个更大元素 I`**（解耦思维：大数组单调栈做“批发”，小数组去字典做“零售” $O(1)$ 对号入座）
  3. **`438. 字母异位词`**（动态账本 `Counter` 记账，归零必 `del` 彻底抹除）
  4. **`567. 字符串的排列`**（438 题的孪生兄弟，固定窗口哈希高频对比）

### 🏷️ Tag 2: 双指针 / 滑动窗口 (Two Pointers / Sliding Window) ── 7 道题
* **核心内功**：在线性时间 $O(n)$ 内，用指针的移动或者边界的缩放收割全场。
* **战果清单**：
  * **同向双指针（快慢指针）**：
    1. **`141. 环形链表`**（操场套圈流，兔子步长 2 追尾乌龟）
    2. **`202. 快乐数`**（隐式套圈，计算轨迹变成单链表环判定）
    3. **`283. 移动零`**（质检员 `fast` 抓非 0 元素，搬运工 `slow` 原地控坑交换）
  * **相向双指针（对撞指针）**：
    4. **`167. 两数之和 II`**（升序排列自带黄金导航，大矮对撞两头夹击）
    5. **`11. 盛最多水的容器`**（木桶效应贪心收缩，无脑挪矮柱子去赌更高的未来）
    6. **`15. 三数之和`**（细节狂魔，排序去重，固定一数后用对撞指针两头包抄）
    7. **`42. 接雨水`**（11 题的二维魔改版，哪边矮结算哪边，原地的极致空间优化）

### 🏷️ Tag 3: 不定长滑动窗口 (Sliding Window - Variable Size) ── 2 道题
* **核心内功**：右指针扩张（Bulk Up），左指针根据约束条件被迫收缩（Weight Loss）。
* **战果清单**：
  1. **`3. 无重复字符的最长子串`**（`\ge left` 护体法则，死锁左指针绝不倒退）
  2. **`209. 长度最小的子数组`**（3 题的反向魔改版，寻找满足条件区间的最小值）

### 🏷️ Tag 4: 单调栈 / 栈与队列 (Monotonic Stack / Stack & Queue) ── 4 道题
* **核心内功**：利用特定的数据结构铁律（先进后出 LIFO / 先进先出 FIFO），操纵时空或者寻找“右边第一个大佬”。
* **战果清单**：
  1. **`735. 小行星碰撞`**（消消乐栈，括号匹配变种，处理正负方向对冲消除）
  2. **`739. 每日温度`**（单调递减栈教科书母题，小黑屋存天数下标，大个子进门轰人算天数差）
  3. **`503. 下一个更大元素 II`**（单调栈 + 环形数组，用 `i % n` 取模技巧在脑子里把数组虚拟转两圈）
  4. **`456. 132 模式`**（单调栈的终极 Boss 题，倒序遍历，用 `ak_2` 继承被轰走的小弟来极限抬高门槛）

### 🏷️ Tag 5: 二叉树与递归 (Binary Tree / DFS / BFS) ── 5 道题
* **核心内功**：打破人肉模拟，掌握大局观的“甩手掌柜流”与“横向水波纹平推法”。
* **战果清单**：
  * **深度优先搜索 (DFS / 递归)**：
    1. **`104. 二叉树的最大深度`**（问左右要结果 $\rightarrow$ 挑个大的 $\rightarrow$ 加个 1 往上交）
    2. **`226. 翻转二叉树`**（镜像照镜子，左右子树无脑对调）
    3. **`100. 相同的树`**（多路同步安检，三道防线卡死数值与结构）
    4. **`101. 对称二叉树`**（镜像探戈舞步，利用 `check` 辅助函数拓展双参数实现外侧对外侧、内侧对内侧）
  * **广度优先搜索 (BFS / 队列)**：
    5. **`102. 二叉树的层序遍历` & `103. 二叉树的锯齿形层序遍历`**（外层大轮盘用 `deque` 横向平推，必须提前拍照锁死 `queue_size` 分发当层入场券；103 题更是利用了 `appendleft()` 的 $O(1)$ 头部插入超能力优雅实现原地蛇形反转）

### 🏷️ Tag 6: 链表高阶微操与二叉搜索树 (Linked List & BST) ── 3 道题
* **核心内功**：在内存里玩指针的断开与缝合，利用“左小右大”的黄金属性极速二分。
* **战果清单**：
  1. **`700. 二叉搜索树中的搜索`**（利用 BST 天生自带的“二分查导航”，一路砍掉一半树枝）
  2. **`206. 反转链表` & `92. 反转链表 II`**（经典的三指针原地大挪移，92 题引入 `dummy` 虚拟头节点稳住大局，利用“火车车厢调头法”穿针引线）
  3. **`426. BST 转双向循环链表`**（融合了中序遍历与三指针织网的难题，首尾缝合形成循环链表）
...
```text
[你的算法通关城堡]
 ├── 线性结构: ▓▓▓▓▓▓▓▓▓▓ 100% (双指针、滑动窗口、单调栈全通关！)
 ├── 队列与 BFS: ▓▓▓▓▓▓▓▓▓▓ 100% (横向层序、锯齿蛇形完美收割！)
 ├── 树形与递归: ▓▓▓▓▓▓░░░░ 60%  (基础 DFS 递归熟练，BST 哨所刚建立)
 └── 高阶链表微操: ▓▓▓▓▓░░░░░ 50%  (反转、局部反转拿下，双向环形概念已懂)
```

# 🎯 为什么你手里的 15 题可以“以一当十”？

> 💡 **算法面试的“二八定律”**：
> 大厂高频面试题里，**80% 的题目都是由 20% 的“核心母题”打碎、重组、换个马甲变出来的**。
> 你手里的这 15 题，绝不是随便刷的酱油题，它们全都是各自领域里的**“泰山北斗”**。跟 LeetCode 题库里那几千道题比起来，15 题听上去就像是刚走出了新手村，但你的功底早已今非昔比！
...
### 1. 你觉得你只做了 `739. 每日温度`？
* **真相是**：你其实已经顺带着秒杀了 `Next Greater Element` 这一整门派的题！
* **降维打击**：只要以后题目说“寻找下一个更大/更小”的元素，你直接闭着眼套单调栈模板，两分钟之内就能写完。

### 2. 你觉得你只做了 `141. 环形链表` 和 `202. 快乐数`？
* **真相是**：你已经彻底掌握了**快慢指针（双指针套圈）**的底层超能力！
* **降维打击**：以后遇到“判断数组/路径有没有循环往复”、“寻找链表的中点”、“寻找环的起点”，你的第一反应绝对是优雅地安排兔子和乌龟去赛跑。

### 3. 你觉得你被 `456. 132 模式` 虐得很惨？
* **真相是**：那可是单调栈里正宗的 **Hard 级别隐蔽题**！
* **降维打击**：你连这种需要倒序遍历、用 `ak_2` 卧底极限抬高门槛的变态题都硬生生啃下来了，以后普通的单调栈中等题在你眼里基本就是砍瓜切菜。

---

## 🚀 下一步策略：高强度“堆量”与全面提速

基础框架和最难懂的底层机制（比如**单调栈的轰人流、滑动窗口的指针防倒退边界、二叉树的递归甩手掌柜套娃**）你都已经通过这 15 道母题彻底理顺了。现在你的地基稳得像磐石一样！

地基打好了，接下来咱们就要开启**“疯狂堆量 + 降维打击”**的轰炸模式。既然嫌 15 题少，咱们今天就开始提速！

### 📋 今日开荒方向二选一：

* **选项 A ── 纵向大扫荡**
  * **打法**：锁定你最喜欢的某一个 Tag（比如【二叉树】或者【滑动窗口】）。
  * **目标**：一口气连刷 5 道它的核心变形题，用高强度连续轰炸彻底把这个 Tag 刷吐、吃透。

* **选项 B ── 横向开荒新大陆**
  * **打法**：直接杀进我们之前列出的【未知新大陆】，首选 **【回溯算法 (Backtracking)】**。
  * **目标**：用 `46. 全排列` 当作突破口，体验一下怎么用几行代码穷举出所有可能，去感受“吃后悔药撤销选择”的全新流派冲击。
...
---
    ...
    【第一主大陆：数据结构与线性战区】
    ├── [区域 1] 基础线性阵地 ── 数组、字符串、链表微操 ──── (▓▓▓▓▓▓▓▓░░ 80% 已点亮)
    ├── [区域 2] 滑动窗口与双指针 ── 连续区间与对撞包抄 ── (▓▓▓▓▓▓▓▓▓▓ 100% 满级)
    └── [区域 3] 栈与队列大本营 ── 单调栈轰人与消消乐 ──── (▓▓▓▓▓▓▓▓▓▓ 100% 满级)
            │
            └── 🌲 穿过无尽树林，开启高维空间...
                    │
    【第二主大陆：树形、图论与搜索战区】
    ├── [区域 4] 二叉树与递归 ── DFS 甩手掌柜与多路同步 ── (▓▓▓▓▓▓░░░░ 60% 攻占中)
    ├── [区域 5] 广度优先 (BFS) ── 队列水波纹地毯式平推 ── (▓▓▓▓▓▓▓▓▓▓ 100% 满级)
    ├── [区域 6] 二叉搜索树 (BST) ── 极速查找的光明顶 ──── (▓▓▓▓░░░░░░ 40% 刚建立)
    └── [区域 7] 高阶图论与高级树 ── 并查集、Trie前缀树 ── (░░░░░░░░░░ 0%  🔒 未解锁)
            │
            └── 🎭 逻辑与工程思维的终极升华，踏入高阶策略领域...
                    │
    【第三主大陆：高阶高级策略与暴风思维战区】
    ├── [区域 8] 暴力穷举与回溯 (Backtracking) ── 决策树探险与后悔药 ── (░░░░░░░░░░ 0% 🔒 迷雾中)
    ├── [区域 9] 局部最优与贪心策略 (Greedy) ── 斩断过去，白手起家 ──── (░░░░░░░░░░ 0% 🔒 迷雾中)
    ├── [区域 10] 愚公移山与分治 (Divide & Conquer) ── 大任劈半，合体归并 ── (░░░░░░░░░░ 0% 🔒 迷雾中)
    └── [区域 11] 动态规划 (DP) ── 状态转移、记账递推与降维打击 ──── (░░░░░░░░░░ 0% 🔒 迷雾中)

---

## 🧭 那些尚未开启的神秘地图与核心大 Boss

除了你已经通关的部分，大地图上还有三大处于迷雾中的高频核心副本。它们是各大厂技术面（尤其是大厂 Senior/Staff 级别考核）最喜欢用来当压轴题的卡人圣地：

### 1. 🎭 区域 8：回溯算法 (Backtracking) ── 暴力穷举的艺术
* **地图长相**：这里是一棵巨大的**决策树**。每一次往前走都是在做单选题（选衣服、选数字、走迷宫）。
* **核心机制**：**“做选择 $\rightarrow$ 深入递归 $\rightarrow$ 撞南墙 $\rightarrow$ 撤销选择（吃后悔药）”**。它在用极其优雅的递归逻辑实现全量的暴力穷举。
* **镇守 Boss**：`46. 全排列`、`78. 子集`、`51. N皇后`

### 2. 🧗 区域 9：动态规划 (DP / Dynamic Programming) ── 降维打击的极意
* **地图长相**：这里是一排排整齐的**砖墙（dp 数组账本）**。
* **核心机制**：**“人类之所以聪明，是因为我们记账。”** 它的核心思想是绝不重复计算历史子问题。通过写出精妙的“状态转移方程”，让计算机像推多米诺骨牌一样，由底向上硬推到终点。
* **镇守 Boss**：`70. 爬楼梯`、`322. 零钱兑换`、`300. 最长递增子序列`

### 3. 🪓 区域 10：分治算法 (Divide & Conquer) ── 愚公移山，强力合体
* **地图长相**：一个巨大的任务，像被一把巨斧不断劈成两半。
* **核心机制**：**“大任务 $\rightarrow$ 劈成两半 $\rightarrow$ 各自解决 $\rightarrow$ 最后合体”**。面对一亿个乱序数字，硬干会死。分治的思路是：左边 5000 万用递归去排序，右边 5000 万用递归去排序。当两边各自排好序后，大老板用极高的微操手段把两个已经排好序的数组进行“合并（Merge）”。
* **镇守 Boss**：
  1. **`148. 排序链表`**（链表归并排序的巅峰微操，极限压榨空间复杂度到 $O(\log n)$）
  2. **`23. 合并 K 个升序链表`**（利用两两分治合并，将多路合并降维打击成二路合并）

### 4. 🌳 区域 7：高级树与堆 (Advanced Tree & Heap) ── 工业级的高效容器
* **地图长相**：由**前缀树（Trie）**、**并查集（Union-Find）**和**大顶堆/小顶堆（Priority Queue）**组成的现代工业级黑科技。
* **核心机制**：用来极速解决“搜索引擎关键词自动补全”、“朋友圈社交网络连通性”、以及“在 10 亿个数字里找出前 100 个最大的数（Top K 问题）”。
* **镇守 Boss**：`208. 实现 Trie`、`215. 数组中的第 K 个最大元素`

---

## 🗺️ 接下来的最佳通关路径与战术规划

> 💡 **通关铁律**：千万不要胡乱去刷题。刷题的最高境界是**“顺藤摸瓜”**，利用你已经练成的内功，去降维打击相连的副本。

基于你目前的武器库（双指针完美、单调栈毕业、BFS 层序纯熟），最丝滑、最省脑细胞的**“特种兵通关路线”**规划如下：

### 🏃‍♂️ 第一步：杀向【区域 6：二叉搜索树 BST】
* **战术理由**：你刚做完了 700 题，且 426 题的概念已经通了。利用二叉搜索树“左小右大”的天然二分属性，把这块地盘先彻底插满你的旗帜。
* **推荐收割目标**：`98. 验证二叉搜索树`、`235. BST的最近公共祖先`

### 🏃‍♂️ 第二步：彻底打穿【区域 1：高阶链表微操】
* **战术理由**：单链表反转（206）和局部反转（92）你都拿下了，双向和循环的概念你也在 426 里体验过了。这时候趁热打铁，把链表这一门派彻底封印。
* **推荐收割目标**：`19. 删除链表的倒数第 N 个结点`、`21. 合并两个有序链表`、`142. 环形链表 II`（快慢指针找环的入口）

### 🏃‍♂️ 第三步：横向破局，开荒【区域 8：回溯算法】
* **战术理由**：你现在的 DFS 递归大局观（甩手掌柜流）已经在 104、226、101 题里练得极其纯熟了。回溯算法本质上就是带有“撤销动作”的 DFS 树形递归。用你现在的树形内功去打回溯，你会发现上手快得不可思议！
* **推荐突破口**：`46. 全排列`（直接去体验吃后悔药的快感）

---

> 📢 **探险队集结令**：数据结构与算法的完整版图已经全部展开，每条路径的战略价值也分析完毕。接下来的第一步，咱们的探险队打算拔营去向何方？
> 
> * ⚔️ **路线 A**：顺着 BST 极速二分导航的快感，去把 **`98. 验证二叉搜索树`** 给办了？
> * ⚔️ **路线 B**：直接调转枪头直奔全新大陆，用 **`46. 全排列`** 开启回溯算法的消消乐大门？

[你的最佳远征路线]
  树形内功纯熟 ──>【区域 8：回溯算法】(玩转全排列决策树)
                    │
                    └──> 顺藤摸瓜 ──>【区域 10：分治算法】(因为分治本质也是用递归劈开左右子树处理)
                                      │
                                      └──> 终极飞升 ──>【区域 11：动态规划 DP】
