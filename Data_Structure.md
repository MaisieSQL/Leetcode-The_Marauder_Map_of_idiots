...
# 算法就这点东西，数组，链表，栈，队列，哈希表，树，图。为什么会有那么多的leetcode？

## 这是一个非常敏锐的直觉。底层的数据结构（物理存储）确实就这么几种。之所以有几千道 LeetCode，是因为算法面试考察的不是“你会不会存数据”，而是如何处理**“多维度的约束”**。

## 我们可以用统计学的视角来理解：数据结构是 Variables（变量），而算法题是 Complex Functions（复杂函数）。以下是为什么“就这点东西”能变幻出无穷题目的三个核心原因：

### 1. 多个数据结构的“组合拳”很多题目不是单一结构的。LRU Cache：它是 Hash Table + Doubly Linked List。用哈希表实现 $O(1)$ 查找，用双链表实现 $O(1)$ 调整顺序。浏览器历史记录：可能是 Stack + Stack。Google 面试最爱：把一种结构“伪装”成另一种。比如用 Array 来实现一个 Heap。

### 2. 同样的结构，不同的“访问逻辑” (Patterns)即便数据都存在 Array 里，处理它的方式（算法）千差万别：双指针：从两头往中间挤（Two Sum）。滑动窗口：像个放大镜一样在数组上滑（Longest Substring）。二分搜索：每次砍掉一半（Binary Search）。动态规划：把大问题拆成小问题，存进数组里防止重复计算。

### 3. “场景化”的逻辑陷阱LeetCode 题目往往会套上一个马甲：森林里的路径：其实是 Tree 的路径总和。单词接龙：其实是 Graph 的最短路径 (BFS)。
